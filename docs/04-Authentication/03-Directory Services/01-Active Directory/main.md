...
description: "Integrate Active Directory with SafeSquid: set up users and configure LDAP for authentication."
keywords:
...
## Set up Active Directory and integrate with SafeSquid

After installing AD on the machine, set up Active Directory and add users, groups, and computers. Link LDAP Admin with Active Directory, then integrate LDAP with SafeSquid to see all users and user groups in the SafeSquid dashboard.

## Prerequisites
- SafeSquid installed on the system
- LDAP Admin installed; download from: [https://sourceforge.net/projects/ldapadmin/](https://sourceforge.net/projects/ldapadmin/)
...
Enter the password, then click Finish.
...
Enter the Username and the password of the Account of the Windows Server. And click on test connection. If anything goes wrong, then verify that the provided details are correct.
...
To change this, go back, right-click the connection, and click Properties.
...
Fill the following details according to the deployment configuration.

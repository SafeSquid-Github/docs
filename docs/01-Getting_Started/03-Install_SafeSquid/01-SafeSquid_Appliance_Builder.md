...
SAB is a security-hardened Debian 13 (Trixie) ISO used to deploy SafeSquid with an optimized operating system, updated kernel, and all required components.

## Problem Statement

Deploying a Secure Web Gateway requires installing the operating system, ensuring it is up to date, applying security patches, configuring DNS services, and setting up supporting software. This process can be disrupted due to system issues, lack of connectivity, or incomplete configurations, leading to failed or inconsistent deployments.

Additionally, recovery from failed installations often requires manual intervention, such as reconfiguring storage for virtual machines or resetting boot devices for physical systems.

## Key Benefits

SAB simplifies and automates the entire deployment process by installing a secure and optimized Debian-based system along with SafeSquid and its dependencies.

The installation ensures:

1. Deployment of the latest stable Linux kernel and system components
2. Automatic application of security patches
3. Seamless two-stage installation (OS setup followed by SafeSquid deployment)
4. Automated configuration of Bind9 as a local caching DNS service
5. Optimized platform setup for performance and stability

The process is designed to be reliable and repeatable. In case of interruption, a clean reinstallation can be easily performed by resetting storage (for virtual machines) or boot devices (for physical systems).

SAB also allows flexibility in DNS configuration, enabling administrators to use default root servers or forward DNS requests to custom name servers, depending on network policies.
...
9. **Wait for installation** (~15 minutes). The preseed script installs all components, then the system reboots.
   ...
   ![Finishing](/img/SAB_Debian/pptx_image16.webp)

10. **Log in** with the default credentials.

    ![Login](/img/SAB_Debian/pptx_image17.webp)

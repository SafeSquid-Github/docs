...
4. Reference the profile in policy modules.
   - Access Control: deny or allow based on server location. See [Access Restriction](/docs/Access%20Restriction/main).
   - Bandwidth Management: prioritize local regions. See [Manage Bandwidth](/docs/Performance%20Accelerators/Manage%20Bandwidth).
   - URL Redirection or Exceptions: adapt experience by region. See [URL Redirection](/docs/URL%20Redirection/main).
   - Reporting: build dashboards by destination country. See [Reporting Module](/docs/Audit%20&%20Forensics/Reporting%20Module).
...
- Real-time logs: confirm `server_country`, `server_region`, and `server_asn` fields. See [Security Logs](/docs/Audit%20&%20Forensics/Security%20Logs).
- Sample curl test:
...
- CDN or anycast endpoints
  ...
  - Fix: allow-list CDNs by ASN where appropriate; apply application signatures. See `../07-Profiling Engine/03-Application Signatures.md`.
- Private or RFC1918 destinations
  - Symptom: no geo data for non-routable IPs
  - Fix: add explicit policy exceptions; rely on identity or application profiles. See [User Identities](/docs/Profiling%20Engine/User%20Identities/main). `./01-User Identities/main.md`.
- DNS-based variance
...
- HTTPS SNI/IP mismatch
  ...
  - Fix: prefer IP-based geo for enforcement; validate SNI with server verification. See [Server Verification](/docs/Server%20Verification/main).

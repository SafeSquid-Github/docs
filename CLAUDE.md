...
### Project Structure
- **Content:** `public/<section>/` (e.g. `public/getting-started/`, `public/guides/`); `blog/` at repo root (date-prefixed posts `YYYY-MM-DD-Title.md`); images under `public/images/` referenced as `/images/category/image-name.webp` (WebP preferred).
- **Infrastructure:** Node ≥20.17, npm, Mintlify CLI (`public/docs.json`); search and GA via Mintlify/docs config. Agent does not change hosting bindings or CI secrets without an explicit ticket.
...
- **Evidence:** Images in `public/images/` → `/images/category/name.webp`; alt text; screenshots and logs per feature. Diagrams: Mermaid preferred; caption or alt. Logs: what to check, where, pattern, success indicator. Code: language tags, comments, expected output when relevant.
...
**Placement:** Docs in `public/<section>/`; blog posts in `blog/` with date prefix (`YYYY-MM-DD-Title.md`); images in `public/images/`; internal links relative.

### Main file (main.md)

**CRITICAL:** Every `public/<section>/` folder must have `main.md` as the navigation hub. Headings follow Voice, structure, and formatting.
...
- **Technical:** Markdown valid; internal links relative and working; images in `public/images/`, paths correct; frontmatter valid YAML; code blocks with syntax highlighting. Screenshots current; code tested or scoped; procedures produce expected results.
...
Node >=20.17.0 (LTS). No `.nvmrc` is present in this repo — pin explicitly:
...
npm is bundled with Node. No global Mintlify install needed.
...
### Available commands (all run from repo root)

```sh
npm run dev            # Local preview → http://localhost:3000
npm run validate       # Mintlify validation — must exit 0 before any PR
npm run broken-links   # Optional: find dead links
```

All three scripts change into `public/` before invoking the `mint` CLI.
**Never run `mint` commands directly** from the repo root or any other directory.

### Mintlify project root

`public/` — `docs.json` lives here; all content folders are children of this directory.

### CI

`.github/workflows/build_and_publish.yml` runs `npm run validate` on every push and PR to `main`, `dev`, and `mintlify-docs`. Production deploy is handled through the Mintlify dashboard (Git integration) — not triggered from this repo's CI.
...
│   ├── docs.json                  ← Navigation, theme, tabs, Mintlify config
│   ├── images/                    ← All images — reference as /images/category/name.webp
│   ├── getting-started/           ← Onboarding and install content
│   ├── guides/                    ← Feature and how-to guides
│   ├── api/                       ← API reference docs
│   ├── architecture/              ← Architecture and concepts
│   ├── cli/                       ← CLI reference
│   ├── faqs/                      ← FAQ pages
│   ├── integrations/              ← Third-party integration guides
│   ├── interface/                 ← UI portal documentation
│   ├── troubleshooting/           ← Symptom-first troubleshooting guides
│   └── use-cases/                 ← Use-case and scenario docs
├── blog/                          ← Date-prefixed blog posts (YYYY-MM-DD-Title.md)
├── .claude/                       ← Canonical agent config directory
...
├── .agents -> .claude             ← Symlink: other agent frameworks resolve to .claude/
├── CLAUDE.md                      ← This file — authoritative guide for all agents and contributors
├── AGENTS.md -> CLAUDE.md         ← Symlink: AGENTS.md-aware frameworks read CLAUDE.md
├── README.md                      ← Human-facing project overview and setup
├── package.json                   ← npm scripts only — do not modify
├── .github/                       ← CI workflows — do not modify without a ticket
└── .gitignore
```

Every content section under `public/` must contain a `main.md` navigation hub.
...
These files control infrastructure, CI, or hosting. Do not modify without an explicit ticket:

- `package.json` — npm scripts and engine constraint
- `package-lock.json` — lockfile; regenerated by `npm install`
- `.github/` — CI workflows and GitHub config
- `public/docs.json` — navigation structure, theme, colors, logo, footer, integrations.
  **Exception:** add new pages to the `navigation` array when adding content — do not touch any other key.
- `public/robots.txt` — search indexing rules
- `.claude/settings.json` — agent runtime environment
- `_old_getting_started_backup/` — archived content; do not reference or edit
...
1. `CLAUDE.md` (this file) — all authoring, structure, voice, and workflow rules
2. `public/docs.json` — understand navigation and existing sections before adding pages
3. The target section folder under `public/` — existing file names, style, and `main.md`

### What to skip

- `.github/` — CI internals; not relevant to content work
- `package.json` / `package-lock.json` — infrastructure; not relevant to content work
- `_old_getting_started_backup/` — archived; do not use as a reference or copy source

### Context budget

`CLAUDE.md` is ~25 KB. Read it fully at session start. Do not re-read on every sub-task — load once and keep in context. For long sessions, re-read only the specific section needed (e.g., Writing Guidelines, Format conventions).

### Agent system bootstrap

**Claude Code** — reads `CLAUDE.md` and `.claude/` directly (both are the canonical files).
Sub-agents are defined in `.claude/agents/`. `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` is set in `.claude/settings.json`.

**Gemini CLI / Gemini Code Assist** — reads `GEMINI.md` if present; falls back to `AGENTS.md`.
`AGENTS.md` is a symlink to `CLAUDE.md` — Gemini reads the same authoritative content.

**GitHub Copilot** — reads `AGENTS.md` (symlink → `CLAUDE.md`).

**All other agent frameworks** — `CLAUDE.md` or `AGENTS.md` (symlink to `CLAUDE.md`) is the canonical instruction file. Read it before taking any action. Do not create framework-specific config files (`GEMINI.md`, `CURSOR.md`, etc.) without a ticket.

### Test expectations

`npm run validate` must exit `0` before any pull request is opened.
CI enforces this on every push — PRs that fail validation will not be merged.
A passing validate is the minimum bar; it does not replace content review.

### Sub-agent workflow

The repo uses a four-agent team for documentation:

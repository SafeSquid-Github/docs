...
Use this page together with [Getting Started](/getting-started/introduction/introduction) when the product does not install cleanly, the web interface is unreachable, or activation cannot be completed.

### Identity and SSL
Use [Authentication](/Authentication), [SSL Inspection](/SSL_Inspection), and [Supporting Services Monit](/getting-started/introduction/interface/supporting-services/Supporting_Services_Monit) when user login, certificate trust, or directory-backed access starts failing.

### Performance and stability
Use [Performance Plot](/Performance_Plot), [Audit & Forensics](/Audit_Forensics), and [Architecture](/getting-started/introduction/architecture) when the proxy becomes slow, unstable, or inconsistent under load.
...
When failures are broad and not isolated to one control, review [Architecture](/getting-started/introduction/architecture), [Audit & Forensics](/Audit_Forensics), and [Supporting Services Monit](/getting-started/introduction/interface/supporting-services/Supporting_Services_Monit) together before escalating.
...
After resolving the issue, verify with [Getting Started](/getting-started/introduction/introduction); for configuration changes see [Configuration Portal](/getting-started/introduction/interface/portals/Configuration_Portal).

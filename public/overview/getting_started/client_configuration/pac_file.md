...
Host a PAC file, point a pilot browser to it, and validate logs before broad rollout with [Enterprise Deployment](/Enterprise_Deployment).
...
Use PAC files when you need flexibility (different rules for different sites) without reconfiguring every browser. For enterprise scale (100+ endpoints), use [Enterprise Deployment](/Enterprise_Deployment) to push PAC URLs via GPO/MDM.
...
1. **Verify your setup** — Confirm traffic flows through SafeSquid
2. **[SSL Inspection](/SSL_Inspection)** — Enable HTTPS decryption
3. **Scale your deployment:**
   - **Enterprise (100+ endpoints):** [Enterprise Deployment](/Enterprise_Deployment) — Push PAC URL via GPO/MDM
   - **Application-specific:** [Application Configuration](/Application_Specific_Configuration) — Configure non-browser apps

...
Apply the OS-specific steps below, test traffic flow, then configure exceptions with [Application-Specific Configuration](/Application_Specific_Configuration) where needed.
...
Use system-wide proxy for complete coverage on a single machine. For enterprise rollout (100+ endpoints), use [Enterprise Deployment](/Enterprise_Deployment) to push settings via GPO/MDM.
...
Some applications (Docker, Git, npm, Python pip) ignore system proxy settings and require [application-specific configuration](/Application_Specific_Configuration).
...
| Some apps ignore proxy | App uses own proxy settings | See [Application-Specific Configuration](/Application_Specific_Configuration) |
...
1. **Verify your setup** — Confirm all application traffic flows through SafeSquid
2. **[Application-Specific Configuration](/Application_Specific_Configuration)** — Configure Docker, Git, npm, and other tools that ignore system proxy
3. **[SSL Inspection](/SSL_Inspection)** — Deploy SafeSquid root CA for HTTPS inspection
4. **Scale to enterprise:** [Enterprise Deployment](/Enterprise_Deployment) — Push system proxy via GPO/MDM for 100+ endpoints

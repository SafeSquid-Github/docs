...
Configure one browser with the steps below, validate traffic in logs, then move to [PAC File](/PAC_File) or [Enterprise Deployment](/Enterprise_Deployment).
...
Use explicit proxy for initial testing. For production deployments, use [PAC File](/PAC_File) or [Enterprise Deployment](/Enterprise_Deployment) instead.
...
- For HTTPS sites: [SSL Inspection](/SSL_Inspection) configured (optional for initial testing)
...
Until [SSL Inspection](/SSL_Inspection) is configured, HTTPS sites will show certificate warnings. This is expected—click through for now.
...
| HTTPS sites don't load at all | SSL inspection not configured | Either [configure SSL inspection](/SSL_Inspection) or disable HTTPS proxy temporarily |
...
1. **Verify your setup** — Run comprehensive tests to confirm proxy functionality
2. **[SSL Inspection](/SSL_Inspection)** — Enable HTTPS decryption to inspect encrypted traffic
3. **Scale your deployment:**
   - **Small team (10-100 users):** [PAC File](/PAC_File)
   - **Enterprise (100+ endpoints):** [Enterprise Deployment](/Enterprise_Deployment)

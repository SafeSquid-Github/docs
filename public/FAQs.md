...
Read more: [Self-Service Portal](/02-SafeSquid_SWG/04-Self-Service_Portal).
...
For more details see the [Support page](/15-Audit_Forensics/main) (license details after activation) or [Activate](/01-Getting_Started/04-Activate).
...
Allow social networking sites in lunch hours: [Allow Social Networking Sites During Lunch Hours](/21-Use_Cases/07-Allow_Social_Networking_Sites_during_Lunch_Hours).
...
For more detail: [Access Restriction](/08-Access_Restriction/main) and [Use Cases](/21-Use_Cases/main) for blocking anonymous proxies and application control.
...
Slowness after sustained operation is often due to resource exhaustion (disk, memory, or connection state). Check disk space and RAM (see [Disk space and RAM are full](/23-Troubleshooting/06-Disk_Space_and_RAM_are_Full)), clear or rotate logs, and review the [Performance Plot](/15-Audit_Forensics/03-Performance_Plot) and [Support](/15-Audit_Forensics/main) page for resource usage. Restart the proxy after freeing resources if needed.
